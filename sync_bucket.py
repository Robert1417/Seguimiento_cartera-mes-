...
def main():
    ...
    tipo_q = df_today[COL_TIPO_ACT].astype(str).str.upper().str.strip()
    ...
    funnel_cols_bucket_names = [
        ...
        if _norm_key(FUNNEL_TO_BUCKET_RENAME.get(c, c))
        not in {_norm_key(x) for x in EXCLUDE_COLS_BUCKET}
    ]
    ...
    header = apply_preferred_order(
        pd.concat([df_bucket, df_out], ignore_index=True)
